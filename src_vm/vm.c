...
static inline void ctss_vm_set_immediate(CTSS_VM *vm,
                                         uint32_t addr,
                                         uint8_t state) {
    ...
    if (next) {
      ...
      CTSS_TRACE_INLINE(
          ("inline candidate: %s (%04x), next: %s (%04x), len: %u\n", hd->name,
           addr, hd2->name, next, len));
      if (len < CTSS_VM_INLINE_THRESHOLD) {
        uint32_t prev = 0;
        addr += 2;
        for (uint8_t i = 0; i < len; i++) {
          CTSS_VMValue word = vm->mem[addr + i];
          if (word.i32 != ctss_vm_cfa_ret || prev == ctss_vm_cfa_lit) {
            CTSS_TRACE_INLINE(("inline: %04x: %08x\n", addr + i, word.i32));
...
void ctss_vm_dump(CTSS_VM *vm) {
  CTSS_PRINT(
      ("dsp: %ld, rsp: %ld, ip: 0x%04x, np: 0x%04x, latest: 0x%04x, "
       "here: 0x%04x, mode: %u, "
       "err: %u\n",
       vm->dsp - vm->ds, vm->rsp - vm->rs, vm->ip, vm->np, vm->latest, vm->here,
       vm->mode, vm->error));
}
