...
uint8_t ctss_process_formant(CTSS_DSPNode *node,
                             CTSS_DSPStack *stack,
                             CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
...
CTSS_DSPNode *ctss_osc_formant_id(char *id,
                                  uint8_t formant,
                                  float freq,
                                  float gain,
                                  float dc,
                                  float smooth) {
  return ctss_osc_formant(id, &formant_osc_fa[formant << 3], freq, gain, dc,
                          smooth);
}

CTSS_DSPNode *ctss_osc_formant(char *id,
                               float *formant,
                               float freq,
                               float gain,
                               float dc,
  ...
  osc->f[7]            = 0.0f;
  node->state          = osc;
  node->handler        = ctss_process_osc_formant;
  return node;
}
...
uint8_t ctss_process_osc_formant(CTSS_DSPNode *node,
                                 CTSS_DSPStack *stack,
                                 CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
