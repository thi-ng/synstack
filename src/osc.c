...
uint8_t ctss_process_osc_sin(CTSS_DSPNode *node,
                             CTSS_DSPStack *stack,
                             CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
...
uint8_t ctss_process_osc_square(CTSS_DSPNode *node,
                                CTSS_DSPStack *stack,
                                CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
  ...
  while (len--) {
    phase +=
        freq + (*lfo++ * state->lfoDepth);  // + (*env++ * state->envDepth);
    TRUNC_PHASE(phase);
    *buf++ = state->dcOffset + (phase < CT_PI ? state->gain : -state->gain);
  }
  state->phase = phase;
  return 0;
}

uint8_t ctss_process_osc_saw(CTSS_DSPNode *node,
                             CTSS_DSPStack *stack,
                             CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
  ...
  while (len--) {
    phase +=
        freq + (*lfo++ * state->lfoDepth);  // + (*env++ * state->envDepth);
    TRUNC_PHASE(phase);
    *buf++ = state->dcOffset + (-phase * CT_INV_PI + 1.0f) * state->gain;
  }
  state->phase = phase;
  return 0;
}

uint8_t ctss_process_osc_tri(CTSS_DSPNode *node,
                             CTSS_DSPStack *stack,
                             CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
  ...
  while (len--) {
    phase +=
        freq + (*lfo++ * state->lfoDepth);  // + (*env++ * state->envDepth);
    TRUNC_PHASE(phase);
    float x = 2.0f - (phase * CT_INV_HALF_PI);
...
uint8_t ctss_process_osc_sawsin(CTSS_DSPNode *node,
                                CTSS_DSPStack *stack,
                                CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
  ...
  while (len--) {
    ...
    TRUNC_PHASE(phase);
    *buf++ = state->dcOffset +
             ((phase > CT_PI) ? -ct_fast_sin(phase) : (phase * CT_INV_PI - 1)) *
                 state->gain;
  }
  state->phase = phase;
  return 0;
...
uint8_t ctss_process_osc_impulse(CTSS_DSPNode *node,
                                 CTSS_DSPStack *stack,
                                 CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
...
float ctss_osc_pblep_spiral(float t, const float dt, const float lfo) {
  return ct_fast_cos(lfo * t * CT_TAU) * t;
}

uint8_t ctss_process_osc_pblep(CTSS_DSPNode *node,
                               CTSS_DSPStack *stack,
                               CTSS_Synth *synth) {
  CTSS_UNUSED(synth);
  CTSS_UNUSED(stack);
  ...
  while (len--) {
    ...
    *buf++ =
        (fn(phase, freq, *lfo++) - ctss_poly_blep(phase, freq)) * state->gain +
        state->dcOffset;
  }
  state->phase = phase;
  return 0;
...
PBLEP_OSC(ctss_process_osc_spiral, ct_fast_cos(*lfo++ *phase *CT_TAU) * phase);
