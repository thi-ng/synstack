...
int main(int argc, char *argv[]) {
    ...
    app.synth.lfo[0] =
        ctss_osc("lfo1", ctss_process_osc_sin, 0.0f, 0.01f, 0.5f, 1.0f);
    app.synth.numLFO = 1;
...
static void init_voice(CTSS_Synth *synth, CTSS_DSPStack *stack, float freq) {
    ...
    CTSS_DSPNode *env =
        ctss_adsr("env", synth->lfo[0], 2.0f, 0.6f, 0.01f, 1.0f, 0.8f);
    CTSS_DSPNode *osc1 =
        ctss_osc_formant_id("osc1", 0, freq, 0.15f, 0.0f, 0.001f);
    CTSS_DSPNode *sum = ctss_op2("sum", osc1, env, ctss_process_mult);
    CTSS_DSPNode *filter = ctss_filter_iir("filter", IIR_HP, sum, NULL,
                                           ctss_randf(300.0f, 1200.0f), 0.8f);
    CTSS_DSPNode *delay =
        ctss_delay("delay", filter, (uint32_t)(SAMPLE_RATE * 3 / 8), 0.0f, 1);
...
static int render_synth(const void *in, void *out, unsigned long frames,
    ...
    if (app->time < 0.006f) {
        float freq =
            ctss_notes[scale[app->noteID % 7] + app->pitch] * pitch[rand() % 3];
        ...
        ctss_calculate_iir_coeff(NODE_ID(s, "filter"),
                                 ctss_randf(300.0f, 5000.0f), 0.7f);
