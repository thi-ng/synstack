...
static const float pitch[]   = {0.25f, 0.5f, 1.0f, 1.0f};
...
static void init_voice(CTSS_Synth *synth, CTSS_DSPStack *stack, float freq) {
  ...
  CTSS_DSPNode *filter =
      ctss_filter_iir("filter", IIR_LP, sum, NULL,
                      ct_smush_minmax(&rnd, 300.0f, 1200.0f), 0.0f);
  CTSS_DSPNode *delay = ctss_delay(
      "delay", filter, (uint32_t)(SAMPLE_RATE * 0.25f * 1.5f), 0.7f, 1);
...
static int render_synth(const void *in,
  ...
  if (ft < 0.008) {
    ...
    NODE_ID_STATE(CTSS_OscState, s, "osc2")->freq = HZ_TO_RAD(freq * 0.501f);
    ...
    ctss_calculate_iir_coeff(NODE_ID(s, "filter"),
                             ct_smush_minmax(&rnd, 100.0f, 5000.0f),
                             ct_smush_minmax(&rnd, 0.1f, 0.6f));
