...
static const float pitch[]    = {0.5f, 1.0f, 0.25f};
...
int main(int argc, char *argv[]) {
  ...
  ctss_add_global_lfo(&app.synth, ctss_osc("lfo3", ctss_process_osc_sin, 0.0f,
...
static void init_voice(CTSS_Synth *synth, CTSS_DSPStack *stack, float freq) {
  ...
  ctss_configure_adsr(env, 0.005f, 0.1f, 0.3f, 1.0f, 0.5f, false);
  CTSS_DSPNode *osc1 =
  ...
  CTSS_DSPNode *delay =
      ctss_delay("delay", pan, (uint32_t)(SAMPLE_RATE * 0.25f * 1.5f), 0.6f, 2);
...
static int render_synth(const void *in,
  ...
  if (ft < 0.008) {
    ...
    NODE_ID_STATE(CTSS_PanningState, s, "pan")->pos = ct_smush_normpos(&rnd);
