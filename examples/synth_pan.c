...
static void init_voice(CT_Synth *synth, CT_DSPStack *stack, float freq) {
    ...
    CT_DSPNode *env =
        ct_synth_adsr("env", synth->lfo[0], 0.0005f, 0.02f, 0.3f, 1.0f, 0.5f);
    ...
    CT_DSPNode *pan = ct_synth_panning("pan", filter, synth->lfo[1], 0.0f);
...
static int render_synth(const void *in, void *out, unsigned long frames,
    ...
    if (ft < 0.008) {
        ...
        freq *= pitch[rand() % 3];
        ct_synth_reset_pluck(NODE_ID(s, "osc1"), freq, 0.01f,
                             ct_randf(0.05f, 0.95f));
        NODE_ID_STATE(CT_PluckOsc, s, "osc1")->gain = ct_randf(0.15f, 0.7f);
        ct_synth_calculate_iir_coeff(NODE_ID(s, "filter"), 18000.0f, 0.1f);
    ...
    if (ft < 0.008) {
        ...
        CT_DSPStack *s = &app->synth.stacks[app->voiceID];
        ct_synth_reset_adsr(NODE_ID(s, "env"));
        ct_synth_reset_pluck(NODE_ID(s, "osc1"), freq, 0.001f,
                             ct_randf(0.05f, 0.95f));
        NODE_ID_STATE(CT_PluckOsc, s, "osc1")->gain = ct_randf(0.6f, 0.9f);
