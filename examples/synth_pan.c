...
static int render_synth(const void *in,
  ...
  if (ft < 0.008) {
    ...
    NODE_ID_STATE(CTSS_PluckOsc, s, "osc1")
        ->gain = ct_smush_minmax(&rnd, 0.15f, 0.7f);
  ...
  if (ft < 0.008) {
    ...
    ctss_reset_pluck(NODE_ID(s, "osc1"), freq, 0.001f,
                     ct_smush_minmax(&rnd, 0.05f, 0.95f));
    NODE_ID_STATE(CTSS_PluckOsc, s, "osc1")
        ->gain = ct_smush_minmax(&rnd, 0.6f, 0.9f);
    ctss_calculate_iir_coeff(NODE_ID(s, "filter"),
                             freq * ct_smush_minmax(&rnd, 0.9f, 1.9f),
                             ct_smush_minmax(&rnd, 0.0f, 0.6f));
    ctss_activate_stack(s);
    app->noteID++;
    app->voiceID = (app->voiceID + 1) % app->synth.numStacks;
  }
