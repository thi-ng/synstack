...
int main(int argc, char *argv[]) {
  ...
  ctss_add_global_lfo(&app.synth,
                      ctss_osc("lfo1", ctss_process_osc_sin, CT_PI * 1.25f,
...
static void init_voice(CTSS_Synth *synth, CTSS_DSPStack *stack, float freq) {
  ...
  ctss_configure_adsr(env, 0.005f, 0.05f, 0.02f, 1.0f, 0.99f, false);
  CTSS_DSPNode *osc1 =
...
static int render_synth(const void *in,
  ...
  if (time - app->time >= 0.1249f) {
    if ((ct_smush(&rnd) % 100) < 80) {
      float freq = ctss_notes[scale[app->noteID % 8] + app->pitch];
      float freq2 =
          ctss_notes[(scale[app->noteID % 8] % 12) + 12 + (app->pitch % 12)];
      ...
      osc->freq            = HZ_TO_RAD(freq * pitch[ct_smush(&rnd) % 3]);
      osc2->freq           = HZ_TO_RAD(freq2);
      NODE_ID_STATE(CTSS_OscState, s, "osc2imp")->phase = 0;
      // for (int i = 0; i < 4; i++) {
      //    formant[i] = ct_smush_minmax(&rnd, 110, 4400);
      //    formant[i + 4] = ct_smush_minmax(&rnd, 0.1f, 2.0f);
      //}
      ctss_set_formant_id(NODE_ID(s, "osc1"), ct_smush(&rnd) % 9);
      // ctss_calculate_iir_coeff(NODE_ID(s, "filter"),
      //                             ct_smush_minmax(&rnd, 300.0f, 2400.0f),
      //                             ct_smush_minmax(&rnd, 0.0f,
      //                             0.73f));
      ctss_calculate_biquad_coeff(NODE_ID(s, "filter"), PEQ,
                                  // ct_smush_minmax(&rnd, 800.0f, 8000.0f),
                                  2400.0f + sinf(time * 0.3f) * 2320.0f, -60.0f,
                                  ct_smush_minmax(&rnd, 0.02f, 0.1f));
      ctss_calculate_biquad_coeff(NODE_ID(s, "filter2"), HPF,
                                  // ct_smush_minmax(&rnd, 800.0f, 8000.0f),
                                  1600.0f + sinf(time * 0.25f) * 1500.0f,
                                  -60.0f, ct_smush_minmax(&rnd, 0.25f, 2.0f));
